...
def load_training_data(data_path):
    """
    Load training data and split into inks and indicators DataFrames.

    Reads a CSV file containing both ink and indicator columns. Columns ending with '_a' are treated as inks,
    and columns ending with '_i' are treated as indicators. The suffixes are removed from the column names
    in the returned DataFrames.

    Parameters
    ----------
    data_path : str
        Path to the input CSV file containing both indicators and inks.

    Returns
    -------
    inks_df : pandas.DataFrame
        DataFrame containing columns corresponding to inks (columns ending with '_a').
        The column names have the '_a' suffix removed.
    inds_df : pandas.DataFrame
        DataFrame containing columns corresponding to indicators (columns ending with '_i').
        The column names have the '_i' suffix removed.
    ...
    KeyError
        If no columns ending with '_a' or '_i' are found.
    """
...
def create_sample_id(inDKs_df):
    """
    Add a 'Sample_id' column and a unified 'name' column to the DataFrame based on matching base sample IDs.

    This function checks that the base sample IDs (the part before the first '_' or '.') in the 'name_i' and
    'name_a' columns match for each row. It then adds a 'Sample_id' column containing this base ID, and a
    'name' column constructed by combining the base ID with the suffix from 'name_i' after the last underscore.

    Parameters
    ----------
    inDKs_df : pandas.DataFrame
        DataFrame containing at least 'name_i' and 'name_a' columns, where each entry is a string
        with a sample identifier possibly followed by an underscore or dot and additional information.

    Returns
    -------
    inDKs_df : pandas.DataFrame
        The input DataFrame with two new columns:
        - 'Sample_id': The base sample ID extracted from 'name_i'.
        - 'name': A unified name constructed as '<Sample_id>_<suffix from name_i>'.

    Raises
    ------
    AssertionError
        If the base sample IDs in 'name_i' and 'name_a' do not match for any row.
    """
